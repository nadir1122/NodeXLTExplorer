...
function save_block(block_data){
  ...
  block.findOne({blockheight: block_data.height}, function(err, foundblock) {
    if(foundblock) {
      //block already exists
    } else {
      let  blockheight=block_data.height;
      let d= block_data.time;
       let blockdate=new Date(d * 1000);
...
module.exports = {
  ...
  check_blocks: function(coin, cb) {
    block.findOne({coin: coin}, function(err, blocks) {
      if(blocks) {
        return cb(true);
      } else {
        return cb(false);
  ...
  create_blocks: function(coin, cb) {
    var newBlocks = new block({
      coin: coin,
    });

    newBlocks.save(function(err) {
      if (err) {
        console.log(err);
        return cb();
      } else {
